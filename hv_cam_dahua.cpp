...
bool HV_CAM_DAHUA::getMatImage(cv::Mat& _Img, uint32_t timeoutMS)
{
	CFrame frame;
	if (!streamPtr)
	{
...
bool HV_CAM_DAHUA::getMonoImage(cv::Mat& _Img, uint32_t timeoutMS)
{
	CFrame frame;
	if (!streamPtr)
	{
	...
	_Img = cv::Mat(frame.getImageHeight(), frame.getImageWidth(), CV_8U, (uint8_t*)frame.getImage());
	/*_Img = image.clone();*/
	return true;
}
